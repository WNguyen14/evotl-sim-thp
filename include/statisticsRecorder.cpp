...
void statisticsRecorder::displayStats() const {
    ...
    for (const auto& pair : statsMap) {
        ...
        double averageFlightTime = (stat.flightsCompleted > 0) ? (stat.totalFlightTime / stat.flightsCompleted) : 0.0;
        double averageDistance = (stat.flightsCompleted > 0) ? (stat.totalDistanceTravelled / stat.flightsCompleted) : 0.0;
        double averageChargeTime = (stat.chargeSessionsCompleted > 0) ? (stat.timeSpentCharging / stat.chargeSessionsCompleted) : 0.0;
        ...
        std::cout << "  Total Number of Faults  : " << stat.totalFlightTime << "\n";
